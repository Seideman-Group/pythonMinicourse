...
### Task 1a: Modify the array by adding (appending) a data point [1000.00,20.00].
...
# # The naive approach
print "naive approach to max wavelengths", max(wavelengths)
...
# #First Pass: Assuming my data is well behaved (that might be a bad assumption),
# #          the wavelength value in my data will be increasing and go beyond the 
# #          the cutoff value for UV.
 
UV_cutoff=380 #in nm

# # What is the index of the row number that corresponds to the IR cutoff?
# # Simple algorithm design:  We will iterate through the data and determine how many rows are in the UV range.
# # Then we use us that number of rows (i) to take a subset of the data and assign that subset to UV_subset.
i=0
for row in data:
        if row[0]<=UV_cutoff:
            ##Once the wavelength value is bigger than the UV_cutoff, then it will stop incrementing
            i=i+1  
...
# # Bonus Task 5: convert your approach for task 4 into a function FindEnergyBounds() in the file HelperFunctions.
...
# # FindEnergyBounds returns two numbers, the idexes that include min and max wavelenghts
# # for the data set.

# # The index numbers are assigned to the return values of FindEnergyBounds.
